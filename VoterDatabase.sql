...
-- a.	Write a query to  see details of male  senior citizen(age above 60)
--  voters  who stays in Sahakar Nagar area of Pune City  [1]

select *
from voter
inner join residenceaddress using(vAddressId)
where city="city8" and area="area8" and voterage>60 and gender="male";

use voterinfo;

-- b.	Write query to fetch City wise total male and total female voters. [1]

select city,sum(case when voter.gender="male" then 1 else 0 end) as TotalMale,
sum(case when voter.gender="female" then 1 else 0 end) as TotalFemale
from voter
inner join residenceaddress using(vaddressId)
group by city;
...
-- Change Booth ID of All voters from Parvati Voting area to 1113.[1]
update  votingmapper
set boothId=(select boothId from boothdetails where bootharea="BoothArea1113")
where boothId=
(select boothId from boothdetails where bootharea="Parvati Nagar")
;

-- c. Delete VotingMapper records if Election Date 
--   is before  Jan 2019 for Pune city.	[1]

 delete from votingmapper
 where year(electiondate)<2019 and month(electiondate)=01 and  
boothId=(select boothid from boothdetails where city="Pune");
